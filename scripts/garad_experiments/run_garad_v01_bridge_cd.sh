...
# GARA-D v0 bridge-CD synthetic smoke runner.
# 这是研究脚本：用于比较不同 lambda_cd 和 adapter 结构的学习信号，不生成正式提交包。
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$SCRIPT_DIR"
BASE_OUT="analysis/garad_v01_bridge_cd_20260518"
mkdir -p "$BASE_OUT"
for cdw in 0.1 1.0; do
  # 三个模型共享同一组 synthetic 设置，便于判断提升来自模型结构还是训练目标。
  tag="cd${cdw//./p}"
  COMMON=(--cpu --base-mode paired --train-limit 64 --eval-limit 32 --num-points 512 --batch-size 2 --base-alpha 0.08 --max-step 0.020 --lambda-cd "$cdw" --lambda-delta 1.0 --lambda-offset 0.0 --steps 160 --log-every 40)
...
base=Path('analysis/garad_v01_bridge_cd_20260518')
rows=[]
# 汇总每个 run 的 summary.json，生成一个便于横向对比的 CSV。
...
out=base/'bridge_cd_summary.csv'
